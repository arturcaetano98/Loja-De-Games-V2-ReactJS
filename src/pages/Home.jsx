...
function Home() {
  ...
  const paginas = Math.ceil(itemLista.length / itemsPorPagina); // Math.ceil arredonda o número para "cima"...
  ...
  const itensAtual = itemLista.slice(inicioIndice, fimIndice);

  // O "ConstroiLista" deve funcionar com o "itensAtual" ao invés do "itemLista".
  const ConstroiLista = () => itensAtual?.map((itemDaLista, key) => {
